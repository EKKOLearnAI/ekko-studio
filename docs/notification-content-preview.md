...
The first-user fallback title is now parsed from the complete structured App message before applying the display bound. Previously SQLite truncated the raw JSON first, making it unparsable and exposing a JSON prefix as the notification title. Only visible `text` parts are eligible; attachment paths and image metadata are excluded.
