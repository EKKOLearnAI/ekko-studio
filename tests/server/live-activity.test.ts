...
describe('Studio Live Activity plan-trigger policy', () => {
 ...
 it('starts plans for separate runs without a per-destination cooldown',async()=>{const consume=await setup();await consume(event('chat.plan.updated','run-a'));await consume(event('chat.plan.updated','run-b'));expect(fetchMock).toHaveBeenCalledTimes(2);expect(fetchMock.mock.calls.map(([,r])=>JSON.parse(r.body).event)).toEqual(['start','start'])})
