...
const validation = workflow.jobs.publish.steps.find((step: any) => step.id === 'release')
const source = validation.run.split("node <<'NODE'\n")[1].split('\nNODE')[0]
const dirs: string[] = []
afterEach(() => dirs.splice(0).forEach(dir => rmSync(dir, { recursive: true, force: true })))

function validate(version: string, tag: string, prerelease = false, name = 'hermes-web-ui') {
...
describe('npm publish release validation', () => {
  ...
  it('keeps prerelease versions and GitHub prereleases off latest', () => {
    for (const result of [validate('0.7.23-beta.1', 'v0.7.23-beta.1'), validate('0.7.23', 'v0.7.23', true)]) {
      expect(result.status, result.stderr).toBe(0)
      expect(result.output).toBe('dist_tag=next\n')
    }
  })
  ...
  it('rejects another package', () => {
    ...
    expect(result.status).not.toBe(0)
    expect(result.stderr).toContain('Expected the public hermes-web-ui package')
  })
})
