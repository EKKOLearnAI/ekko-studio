...
export class ShareAppIdentityVerifier {
  ...
  async verify(token: string): Promise<VerifiedShareAppIdentity> {
    ...
    const key = createHash('sha256').update(origin.origin).update('\0').update(token).digest('hex')
    const cached = this.cache.get(key)
    if (cached && cached.validUntil > this.now()) return { ...cached }
    this.cache.delete(key)
    let response: Response
    try {
      response = await this.fetcher(new URL('/api/app/auth/me', origin), {
