...
export function clarificationTurnInstruction(contextId: string): string {
  return `<studio_interaction_context>\nWhen a missing user decision materially affects the task, call ekko_studio_clarify from the ekko-studio-interaction MCP server with a concise question and optional choices. This tool displays the existing Studio/App question UI and waits for the user's response. Use it instead of terminal input or native interactive tools unavailable in headless mode. If tools are deferred, search for ekko-studio-interaction / clarify and use the discovered tool name. A timeout, dismissal, or cancellation is not user approval. Never use it from delegated subagents or background tasks.\nCurrent turn context_id="${contextId}". Use only this interaction context, never an id from earlier messages.\n</studio_interaction_context>`
}
