...
export function createApi(request: typeof defaultRequest = (...args) => defaultRequest(...args)) {
  async function startLocalSttStream(): Promise<{ sessionId: string }> {
    return request<{ sessionId: string }>('/api/studio/stt/local-stream', {
      method: 'POST',
    })
  }

  async function pushLocalSttStreamChunk(sessionId: string, audio: Blob): Promise<LocalSttStreamResponse> {
  ...
  async function finishLocalSttStream(sessionId: string): Promise<LocalSttStreamResponse> {
    return request<LocalSttStreamResponse>(`/api/studio/stt/local-stream/${encodeURIComponent(sessionId)}/finish`, {
      method: 'POST',
    })
  }

  async function cancelLocalSttStream(sessionId: string): Promise<{ success: true }> {
    return request<{ success: true }>(`/api/studio/stt/local-stream/${encodeURIComponent(sessionId)}`, {
      method: 'DELETE',
    })
  }

  async function transcribeSpeech(req: TranscribeSpeechRequest): Promise<TranscribeSpeechResponse> {
    if (!req.provider) {
  ...
  return { startLocalSttStream, pushLocalSttStreamChunk, finishLocalSttStream, cancelLocalSttStream, transcribeSpeech }
}

export const { startLocalSttStream, pushLocalSttStreamChunk, finishLocalSttStream, cancelLocalSttStream, transcribeSpeech } = createApi()
