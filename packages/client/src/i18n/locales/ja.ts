...
export default {
  jev: {
    ...
    memoryOptionsHint: "保存した変更は次の実行から有効です。無効時や JEV が利用できない場合は元の処理を維持します。",
    memoryDisabled: "無効（個別設定は保持）",
    memoryNoFeatures: "拡張機能が選択されていません",
    memoryReady: "選択した拡張機能が有効",
    memoryKindRouting: "意味によるカテゴリ選択",
    memoryKindRoutingHint: "キーワード検索で見つからない関連メモリカテゴリを補います。",
    memoryRerank: "候補メモリの再ランキング",
    memoryRerankHint: "コンテキストのトークン予算を適用する前に関連度順に並べます。",
    memoryWriteReview: "メモリ書き込みの検証",
    memoryWriteReviewHint: "一括書き込みの確定前にユーザーの根拠、持続性、カテゴリを確認します。",
    memoryRelevanceFilter: "無関係な記憶の除外",
    memoryRelevanceFilterHint: "通常の記憶を個別に判定し、必要な制約を保持します。判定失敗時は元の検索結果に戻します。",
    memoryFilterMinConfidence: "除外判定の確信度",
    memoryFilterMinConfidenceHint: "0.5～1、既定値 0.8。この信頼度以上で無関係と判定された記憶だけを除外し、不確かな記憶は保持します。",
    memoryAdvanced: "詳細パラメータ",
    memoryCandidateLimit: "検索候補数の上限",
    memoryCandidateLimitHint: "各段階で最大 1～50 件の記憶を評価。推奨値は 20 件。",
    memoryRecallMinConfidence: "検索のしきい値",
    memoryRecallMinConfidenceHint: "0.5～1、既定値 0.5。高いほど選択する分類が減り、順位付けに高い確信度が必要です。",
    memoryMinConfidence: "書き込み検証の確信度",
    memoryMinConfidenceHint: "0.5～1、既定値 0.8。検証が不確かな場合は元の書き込み処理を維持します。",
    memoryTimeout: "メモリ拡張の合計待機時間（ミリ秒）",
    memoryTimeoutHint: "100～30000、推奨値は 3000。召回または一括書き込みごとの合計上限。各リクエストには接続タイムアウトも適用されます。",
  ...
  chat: {
    ...
    editQueuedMessage: 'キューのメッセージを編集',
    saveQueuedMessageEdit: 'キューのメッセージの編集を保存',
    cancelQueuedMessageEdit: 'キューのメッセージの編集をキャンセル',
